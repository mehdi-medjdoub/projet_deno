...
const dataResponse = (
  ctx: RouterContext,
  status: number = 500,
  data: any = { error: true, message: "Processing error" },
) => {
...
const deleteMapper = (data: any, mapperNameRoute?: string): any => {
  mapperNameRoute !== "getSongs" && mapperNameRoute !== "getSong"
    ? (delete data.id)
    : null; // ! l'ordre est important pour le rename de la key _id en id
  mapperNameRoute !== "getBills"
    ? (delete data._id)
    : (data = renameKey(data, "_id", "id")); // !
  delete data.userdb;
  ...
  delete data.idUser;
  data = mapperNameRoute === "newChild"
    ? renameKey(data, "_role", "role")
    : data;
...
const passwordFormat = (data: string): Boolean => {
  ...
  let regexPassword =
    /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#^-])[A-Za-z\d@$!%*?&#^-]{7,20}$/; //maj mini specialchar chiffre taille7_mini taille20_max
  return (data.match(regexPassword) == null ||
      data === undefined /*|| !isValidLength(data, 7, 20)*/)
    ? false
    : true;
};
 ...
 */
const getChildsByParent = async (
  userParent: UserInterfaces,
): Promise<Array<UserInterfaces>> => {
 ...
 */
const randomFloat = (min: number | float, max: number | float) =>
 ...
 */
const calculTtcToHt = (
  montant_ttc: number | float,
  tauxTva: number | float,
) => {
...
const initFiles = async (directory: string = "upload") => {
  ...
  for await (
    const data of Deno.readDirSync(Deno.cwd().concat("/" + directory))
  ) {
...
const stockFile = async (name: string, directory: string = "upload") => {
  ...
  let extName =
    extNamePath === "" || extNamePath === null || extNamePath === undefined
      ? ""
      : extNamePath;
 ...
 * @param {UserInterfaces} userParent userParent
 */
const updateSubscriptionChilds = async (
  userParent: UserInterfaces,
): Promise<void> => {
  let childs: Array<UserInterfaces> = await getChildsByParent(userParent);
  let userChild: any;
  for (let i = 0; i < childs.length; i++) {
    userChild = new UserModels(
      childs[i].email,
      childs[i].password,
      childs[i].lastname,
      childs[i].firstname,
      childs[i].dateNaissance,
      childs[i].sexe,
      childs[i].attempt,
      childs[i].subscription,
    );
    userChild.setId(<{ $oid: string }> childs[i]._id);
    await userChild.update(
      { subscription: 1 /*, dateSouscription: new Date()*/ },
    );
  }
};
 ...
 */
const initProductStripe = async (
  name: string = "Radio-FEED",
  description: string = "Radio pour les enfants",
) => {
  if (await new ProductDB().count({ name: name }) === 0) {
    ...
    await new ProductDB().insert(
      { idProduct: responseAddProduct.data.id, name: name },
    );
...
const isValidDateCard = (data: any): boolean => {
  ...
  month = parseInt(month) > 0 && parseInt(month) < 10
    ? "0".concat(String(parseInt(month)))
    : month;
  let verifYear: boolean = data.year > year ? true : false;
  let isValidDate: boolean = data.year === year
    ? parseInt(data.month) >= parseInt(month) ? true : false
    : verifYear;
  return isValidDate;
};
